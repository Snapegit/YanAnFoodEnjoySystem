	...
	<div class="app-contain" :style='{"padding":"0px 0","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"80%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'转发')" @click="meishizhuanfaonAcross('转发','meishizhuanfa','是','','')" type="warning">转发</el-button>
					<el-button class="approval_btn" v-if="btnAuth('meishixinxi','审核')" type="warning" @click="approvalClick()">审核</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('meishixinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('meishixinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const meishizhuanfaonAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
