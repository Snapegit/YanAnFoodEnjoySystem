	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 0 40px","margin":"0 auto","overflow":"hidden","borderRadius":"4px","textAlign":"center","background":"#fff","width":"80%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('meishizhuanfa','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.meishitupian && item.meishitupian.substr(0,4)=='http'" @click.stop="preViewClick(item.meishitupian)">
							<el-image class="data_img" :src="item.meishitupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.meishitupian.split(',')[0])">
							<el-image class="data_img" :src="item.meishitupian?$config.url + item.meishitupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/meishizhuanfaAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.meishileixing = categoryList.value[categoryIndex.value]
		}
	...
	const detailClick = (id) => {
		router.push('meishizhuanfaDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
